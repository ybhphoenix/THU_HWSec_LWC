...
{
    memset(state, 0, NLANES*sizeof(tXoodooLane));  //NLANES = 12 lane的数量 //tXoodooLane为 uint32_t 
}

/* ---------------------------------------------------------------- */
//void指针指向的数据类型未定，将其值赋给其他值时要类型转换，但是任何类型的指针都可以直接赋值给void*
void Xoodoo_AddByte(void *state, unsigned char byte, unsigned int offset)  //给state的指定8 bit(offset确定的)异或一个数据 
{
    assert(offset < NLANES*sizeof(tXoodooLane));  //如果条件为真则执行下面的语句 
...
void Xoodoo_AddBytes(void *state, const unsigned char *data, unsigned int offset, unsigned int length)
{
    ...
    for(i=0; i<length; i++)
        ((unsigned char *)state)[offset+i] ^= data[i];  //给state的指定8 bit(offset确定的)异或i个数据
}
...
void Xoodoo_OverwriteBytes(void *state, const unsigned char *data, unsigned int offset, unsigned int length)
{
    assert(offset < NLANES*sizeof(tXoodooLane));
    assert(offset+length <= NLANES*sizeof(tXoodooLane));
    memcpy((unsigned char*)state+offset, data, length);  //从state + offset地址开始复制 length 个data 
}
...
void Xoodoo_ExtractBytes(const void *state, unsigned char *data, unsigned int offset, unsigned int length)  //提取数据 
{
    assert(offset < NLANES*sizeof(tXoodooLane));
    assert(offset+length <= NLANES*sizeof(tXoodooLane));
...
static void fromBytesToWords(tXoodooLane *stateAsWords, const unsigned char *state)
{
    ...
    for(i=0; i<NLANES; i++) {
        stateAsWords[i] = 0;
        for(j=0; j<sizeof(tXoodooLane); j++)  //sizeof返回字节数 
...
static void Xoodoo_Round( tXoodooLane * a, tXoodooLane rc )
{
    ...
    for (x=0; x<NCOLUMS; ++x)
        e[x] = ROTL32(p[(x-1)%4], 5) ^ ROTL32(p[(x-1)%4], 14);  //注意这里是x-1和文章中的结果是一样的  
    for (x=0; x<NCOLUMS; ++x)
        for (y=0; y<NROWS; ++y)
            a[index(x,y)] ^= e[x];
    Dump("Theta", a, 2);
